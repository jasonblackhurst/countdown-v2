...
class _WsClient {
  ...
  Future<Map<String, dynamic>> nextMsg(
    ...
    return _ctrl.stream.firstWhere(predicate).timeout(timeout).then((m) {
...
void main() {
  group('E2E: two-player game', () {
    ...
    test('two players join, vote 1 card each, highest card is played first', () async {
      // ── Connect Alice ──────────────────────────────────────────────────────
      ...
      final alicePlayers = aliceRoundState['state']['players'] as List<dynamic>;
      ...
      final bobEntry =
          bobPlayers.firstWhere((p) => ((p as Map)['hand'] as List).isNotEmpty)
              as Map<String, dynamic>;
        ...
        for (var roundIndex = 0; roundIndex < cardCounts.length; roundIndex++) {
          ...
          for (var i = 0; i < allCards.length; i++) {
            ...
            if (isLastRound && i == allCards.length - 1) {
              ...
              expect((wonState['state']['discard_pile'] as List).length, 100);
    ...
    test(
      'two players play a perfect game and win with all 100 cards discarded',
      () async {
        // ── Connect Alice ──────────────────────────────────────────────────────
        final alice = await _WsClient.connect(port);
        alice.send({'type': 'create_room'});
...
int _myHandCard(Map<String, dynamic> stateUpdate) {
  ...
  final myEntry =
      players.firstWhere((p) => ((p as Map)['hand'] as List).isNotEmpty)
          as Map<String, dynamic>;
